...
export default function Social() {
  return (
    <section id="social" className={styles.social} aria-label="Social Presence">
      <JellyBlobs configs={JELLY} />
      <Reveal as="div" className={styles.title}>
        <div className="s-label" style={{ justifyContent: 'center' }}>Connect</div>
        <h2 className="s-head">Social Presence</h2>
      </Reveal>
      <div className={styles.grid}>
        <IsoCard href={socialLinks.github} label="GitHub" name="GitHub" handle="@Prabuddha747" accent="rgba(255,255,255,.05)" delayCss="0s" icon={GITHUB_SVG} />
        <IsoCard href={socialLinks.linkedin} label="LinkedIn" name="LinkedIn" handle="Prabuddha Verma" accent="rgba(10,102,194,.1)" delayCss=".3s" icon={LINKEDIN_SVG} />
        <IsoCard href={socialLinks.instagram} label="Instagram" name="Instagram" handle="@prabuddha_verma" accent="rgba(225,48,108,.08)" delayCss=".6s" icon={INSTAGRAM_SVG} />
      </div>
    </section>
  );
}
